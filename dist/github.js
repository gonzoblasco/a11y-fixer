 ...
 */
export function setCheckStatus(state, description) {
    if (!checkGh()) {
    ...
    try {
        execSync(`gh api repos/${ownerRepo}/check-runs --field name="${title}" --field head_sha="${sha}" --field status=completed --field conclusion="${state}" --field output:title="${title}" --field output:text="${description}" --field output:summary="${description}"`, { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'ignore'] });
    }
    catch (error) {
