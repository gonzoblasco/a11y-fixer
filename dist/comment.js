 ...
 */
export function generateComment(result, evolution, config) {
    const sections = [];
    // 1. Badge
 ...
 */
function getBadge(result, evolution, config) {
 ...
 */
function getSummary(result, evolution) {
    if (result.totalCount === 0) {
    ...
    if (persistentCount > 0)
        parts.push(`${persistentCount} persistent`);
 ...
 */
function getImpactPrefix(impact) {
    switch (impact) {
        case 'critical':
            return '🔴 CRITICAL';
        case 'serious':
